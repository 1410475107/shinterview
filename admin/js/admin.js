...
function addcate() {
    //事件源
    let addcate = document.querySelector('.addcate');
    if (!addcate) return;

    //事件三要素
    addcate.onclick = function () {
        // 要提交的数据
        let data = '';
        //姓名必填 2-4汉字
        let cname = document.querySelector('input[name="cname"]');
        if (!cname.value) {
            cname.parentNode.nextElementSibling.classList.add('H');
            cname.focus();
            return false;
        } else {
            cname.parentNode.nextElementSibling.classList.remove('H');
        ...
        let xhr = new XMLHttpRequest();
        // 第二步：建立对服务器的请求
        xhr.open('POST', './addcatesubmit.php');
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        //第三步：发送请求
        xhr.send(data);
        //监听状态改变
